...
public class ForecastData {
    ...
    public static String[] getDateTimes(List<ForecastData> forecasts) {
        String[] dateTimes = new String[forecasts.size()];
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm");
        for (int i = 0; i < forecasts.size(); i++) {
            dateTimes[i] = forecasts.get(i).getDateTime().format(formatter);
        }
        return dateTimes;
    }

    public static String[] getWeathers(List<ForecastData> forecasts) {
        String[] weathers = new String[forecasts.size()];
        for (int i = 0; i < forecasts.size(); i++) {
            weathers[i] = forecasts.get(i).getWeather();
        }
        return weathers;
    }
